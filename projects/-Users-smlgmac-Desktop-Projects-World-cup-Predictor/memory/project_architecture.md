...
4. **Train ML**: `python -m ml.train` → 5 models in `models/`, registered in `ml_models`
5. **Predict**: `POST /api/v1/ml/predict` → statistical + per-model ML + ensemble + SHAP
6. **WC2026 sim**: `POST /api/v1/world-cup/simulate` → Monte Carlo using official groups and placeholder knockout slots
